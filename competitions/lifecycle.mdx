...
Every competition moves through the same states, in the same order, no exceptions. Know the lifecycle and you know exactly when you can join, when the clock starts, and when your payout lands.
...
A competition is either **PENDING → LIVE → ENDED**, or it's **PENDING → CANCELLED**. There's no other path.
...
<Steps>
  <Step title="Join">
    ...
    Free competitions skip the payment step entirely — no permit, no entry fee. Open free competitions just need you logged in to join; token-gated free competitions add a one-time Solana wallet signature to prove you hold \$SPEC.
  </Step>
  <Step title="Fill and start">
    A competition stays **PENDING** until its lobby fills to max participants. The moment it fills, the countdown begins: it goes **LIVE one hour later**, giving late joiners a last window and everyone else time to get ready.
